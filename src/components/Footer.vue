...
// State Nomor HP & WhatsApp
const phoneNumber = ref("");
const defaultMessage = encodeURIComponent(
  "Halo Minuman Murah, saya ingin bertanya tentang layanan atau produk."
);
...
const whatsappUrl = computed(() => {
  return sanitizedPhone.value
    ? `https://wa.me/${sanitizedPhone.value}?text=${defaultMessage}`
    : "#";
});
...
// Fetch Konfigurasi Publik
const fetchFooterConfigs = async () => {
  try {
    const [igRes, ttRes, fbRes, phoneRes] = await Promise.allSettled([
    ...
    if (phoneRes.status === "fulfilled") {
      phoneNumber.value =
        phoneRes.value?.data?.data?.casted_value ||
        phoneRes.value?.data?.data?.value ||
        "";
    }
  } catch (error) {
    console.error("Gagal mengambil konfigurasi footer:", error);
  }
};
...
<template>
  <footer class="w-full bg-black pt-12 pb-6 border-t border-gray-800 text-gray-300">
        ...
        <div class="md:col-span-1">
          ...
          <ul class="space-y-2 text-xs text-gray-400">
            <li v-for="item in footerNav.bantuan" :key="item.name">
              ...
              <!-- Link Internal Vue Router -->
              <router-link v-else :to="item.href" class="hover:text-white transition-colors">
                {{ item.name }}
              </router-link>
            </li>
          </ul>
          ...
          <div>
            ...
            <div class="flex items-center gap-3.5 text-white">
              ...
              <a :href="socialLinks.instagram" target="_blank" rel="noopener noreferrer"
                class="hover:text-[#D4AF37] transition-colors" title="Instagram">
              ...
              <a :href="socialLinks.tiktok" target="_blank" rel="noopener noreferrer"
                class="hover:text-[#D4AF37] transition-colors" title="TikTok">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path
              ...
              <a :href="socialLinks.facebook" target="_blank" rel="noopener noreferrer"
                class="hover:text-[#D4AF37] transition-colors" title="Facebook">
                <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 24 24">
                  <path
