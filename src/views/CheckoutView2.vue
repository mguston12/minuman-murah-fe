...
const {
    ...
    subtotal,
    () => cartItems.value.map((item) => item.variant_id),
);
        ...
        if (publicConfigService?.getProtection) {
            ...
            if (data) {
                protectionConfig.value = {
                    ...
                    description:
                        data.description || "Melindungi barang dari kerusakan & kehilangan.",
                };
            }
        }
    } catch (err) {
    ...
    <div class="min-h-screen bg-[#FAF6F0] py-8 px-4 sm:px-6 lg:px-8 font-sans text-gray-900">
                    ...
                    <div v-if="cartItems.length > 0" v-for="group in groupedByStore" :key="`store-${group.store_key}`"
                            ...
                            <div class="flex items-center justify-between mb-3">
                                <div class="flex items-center gap-4">
                                    ...
                                    <div>
                                        ...
                                        <p v-if="notePerItem[item.variant_id]"
                        ...
                        <div class="flex items-center justify-between pt-2 border-t border-gray-100">
                            <div>
                                ...
                                <p v-if="shippingPerStore[group.store_key]?.agent"
                            ...
                            <button type="button" @click="openShippingModalForStore(group.store_key)"
                                class="text-sm font-bold text-[#E25C38] hover:underline cursor-pointer">
                                {{
        ...
        <div v-if="showShippingModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
            <div class="bg-white rounded-2xl max-w-md w-full p-6 space-y-4 shadow-xl">
                ...
                <div v-else-if="currentStoreShippingOptions?.length > 0"
                    class="space-y-2 max-h-72 overflow-y-auto pr-1">
                    <div v-for="(opt, idx) in currentStoreShippingOptions" :key="idx"
                        @click="selectCourierOptionFromModal(opt)" :class="[
            ...
            <div class="bg-white rounded-3xl max-w-xl w-full p-6 space-y-5 shadow-2xl relative my-8">
                ...
                <form @submit.prevent="submitAddAddress" class="space-y-4">
                    <div>
                        ...
                        <div class="flex gap-2">
                            <button type="button" v-for="opt in labelOptions" :key="opt"
                                @click="addressForm.label_place = opt" :class="[
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <select v-model="selectedCityId" @change="onCityChange"
