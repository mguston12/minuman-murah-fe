<template>
  <div
    class="min-h-[70vh] flex items-center justify-center bg-gray-50 px-4 py-12"
  >
    <div
      class="max-w-md w-full text-center bg-white p-8 rounded-xl shadow-md border border-gray-100"
    >
      <!-- STATE LOADING -->
      <div v-if="status === 'loading'" class="py-6">
        ...
        <p class="text-gray-500 text-sm mt-2">
          Mohon tunggu sebentar, kami sedang memproses akun kamu.
        </p>
      </div>
      ...
      <div v-else-if="status === 'success'" class="py-4">
        ...
        <p class="text-gray-600 text-sm mt-2 mb-6">
          {{
            message ||
            "Akun kamu telah aktif. Silakan login untuk mulai berbelanja."
          }}
        </p>
        <router-link
          to="/login"
...
const processVerification = async () => {
  ...
  try {
    ...
    message.value =
      response?.data?.message || "Email kamu berhasil diverifikasi.";
  } catch (error) {
    console.error("Email verification error:", error);
    status.value = "error";
    message.value =
      error.response?.data?.message ||
      "Gagal memverifikasi email. Token mungkin sudah tidak berlaku.";
  }
};
