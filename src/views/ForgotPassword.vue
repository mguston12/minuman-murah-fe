...
const localError = computed(() => errorMessage.value || "");
  ...
  try {
    ...
    if (apiError || !data?.success) {
      errorMessage.value =
        apiError?.message ||
        data?.message ||
        "Gagal mengirim tautan atur ulang kata sandi.";
    } else {
      successMessage.value =
        "Tautan atur ulang kata sandi telah dikirim! Silakan periksa kotak masuk email Anda.";
      email.value = "";
    }
  } catch (err) {
    if (isMounted) {
        ...
        <form @submit.prevent="handleForgotPassword" class="space-y-4">
          ...
          <div>
            ...
            <p class="mt-1 text-[11px] text-gray-400">
              Tautan reset akan dikirimkan langsung ke email ini.
            </p>
          </div>
          ...
          >
            ...
            {{ isLoading ? "Mengarahkan..." : "Kirim Tautan Reset" }}
          </button>
        </form>
