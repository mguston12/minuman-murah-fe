...
const whatsappMessage = computed(() => {
  ...
  const variantText = selectedVariant.value
    ? ` (${selectedVariant.value.variant_name})`
    : "";
  const storeText = selectedStore.value?.store?.name
    ? ` di toko ${selectedStore.value.store.name}`
    : "";
  return `Halo Minuman Murah, saya ingin membeli produk *${product.value.name}${variantText}* sejumlah ${quantity.value} pcs${storeText}. Harga: Rp ${activePrice.value.toLocaleString(
    "id-ID",
      ...
      if (product.value.variants && product.value.variants.length > 0) {
        ...
        if (
          ...
          const availableStore =
            firstVariant.stock_relations.find((s) => getAvailableQty(s) > 0) ||
